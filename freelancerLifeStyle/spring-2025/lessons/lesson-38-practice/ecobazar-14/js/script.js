...
function documentActions(e) {
  ...
  if (eventType === "click") {
    ...
    if (targetElement.closest(".quantity__button")) {
      ...
      // 	++inputValue
      // }
      button.classList.contains("quantity__button--icon-minus")
        ? --inputValue
        : ++inputValue;
    ...
    if (targetElement.closest(".tabs-product__button")) {
      ...
      if (!tabButton.classList.contains("tabs-product__button--active")) {
        ...
        activeTab
          ? activeTab.classList.remove("body-tabs-product__item--active")
          : null;
        activeNavTab
          ? activeNavTab.classList.remove("tabs-product__button--active")
          : null;

        const navParent = tabButton.parentElement.parentElement;
        tabButton.classList.add("tabs-product__button--active");
        const activeTabIndex = indexInParent(
          navParent,
...
function initCoundownItem(coundownItem) {
  ...
  if (goalTime) {
    const coundownItemSpans = coundownItem.querySelectorAll(
    ...
    let timer = setInterval(() => {
      ...
      if (timeLeft >= 0) {
        ...
        const hours = Math.floor(
          (timeLeft % MSECONDS_PER_DAY) / MSECONDS_PER_HOUR
        );
        const minutes = Math.floor(
          (timeLeft % MSECONDS_PER_HOUR) / MSECONDS_PER_MIN
        );
        const seconds = Math.floor(
          (timeLeft % MSECONDS_PER_MIN) / MSECONDS_PER_SEC
        );
