...
if (confirm("Почати тестування?")) {
  ...
  const sitesTotalPrice: number = sites.reduce(
    (previousSum, site) => previousSum + site.siteCost,
    0
  );
  ...
  function getAllSponsorsList(sites: Site[]): Sponsor[] {
  ...
  function getYearWasBiggestCost(sites: Site[]): number {
    let yearWasBiggestCost: number = 0;

    const biggestCost: number = sites
      .map((site) => site.siteCost)
      .reduce((previousCost, cost) =>
        previousCost > cost ? previousCost : cost
      );
  ...
  document.write(`
    <li class="list__item">
      <span>5) рік, коли прибуток був найбільшим: ${getYearWasBiggestCost(
        sites
      )};</span>
    </li>
  `);
  ...
  const sitesLessCost10000: Site[] = sites.filter(
    (site) => site.siteCost <= 10000
  );

  console.log("7) список сайтів з вартістю до 10000 :>> ", sitesLessCost10000);

  const sitesBiggerCost10000: Site[] = sites.filter(
    (site) => site.siteCost >= 10000
  );

  console.log(
    "7) список сайтів з вартістю більше 10000 :>> ",
    sitesBiggerCost10000
  );
