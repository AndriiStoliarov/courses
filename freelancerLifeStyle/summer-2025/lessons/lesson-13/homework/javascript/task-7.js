...
if (confirm("Почати тестування?")) {
    const NUMBER_FROM = 1, NUMBER_TO = 9;
    let timerId = setInterval(() => {
        additionSimulator();
    }, 10000);
    // функция, яка емулює тренажер додавання.
    function additionSimulator() {
        const leftOperand = generateRandomNumber(NUMBER_FROM, NUMBER_TO);
        const rightOperand = generateRandomNumber(NUMBER_FROM, NUMBER_TO);
        const sumResult = leftOperand + rightOperand;
        const sumResultAnswer = prompt(`Скільки буде ${leftOperand} + ${rightOperand} = ?`);
